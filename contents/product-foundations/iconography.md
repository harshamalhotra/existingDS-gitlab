...
Avoid using different icons to refer to one specific meaning. For example, [pencil](/product-foundations/iconography-directory/?q=~pencil) should be used for editing or updating an item, rather than [pencil-square](/product-foundations/iconography-directory/?q=pencil-square) which is used for composing.
...
- A [remove](/product-foundations/iconography-directory/?q=~remove) icon typically represents permanently deleting an object or, in the case of tokens, revoking access.
- A [lock](/product-foundations/iconography-directory/?q=~lock) icon represents editing restrictions (read-only, file locking) and access restriction (private projects, setting set by admin/parent group).
...
Existing [GitLab SVGs icons](/product-foundations/iconography-directory/) haven't historically followed a naming convention, so you may encounter different patterns until we're able to address them individually. Use the following guidelines for new icons:
...
- You can view all of the current icons at the [GitLab SVGs](/product-foundations/iconography-directory/) site.
