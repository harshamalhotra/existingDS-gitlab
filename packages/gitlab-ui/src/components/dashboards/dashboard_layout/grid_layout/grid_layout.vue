  ...
  watch: {
    ...
    'value.panels': {
      handler(newPanels) {
        ...
        this.gridPanels = this.gridPanels.map((panel) => {
          const updatedPanel = newPanels.find((p) => p.id === panel.id);
          const updatedPanelProps = pickBy(updatedPanel, (k) => k !== 'gridAttributes');

          return {
            ...panel,
            ...(updatedPanel ? { props: { ...updatedPanelProps } } : {}),
          };
        });
      },
      deep: true,
