 ...
 *
 * The jsdom environment does not support changing the current location.
 * This helper uses the documented way of setting the window location:
 * https://github.com/jsdom/jsdom/tree/26.1.0?tab=readme-ov-file#reconfiguring-the-jsdom-with-reconfiguresettings
 *
 ...
 *
 *     setWindowLocation('https://ui.test');
 *     // window.location.href is now 'https://ui.test'
 *
 ...
 * @returns {undefined}
 */
export function setWindowLocation(url) {
  // Sass tests do not use a jsdom environment, so do nothing in that case.
  if (!globalThis.jsdom) return;
  ...
  globalThis.jsdom.reconfigure({ url: newUrl.href });
}
